...
- **Caddy** terminates controller HTTPS while the native Go relay owns authenticated HTTP CONNECT streams. Later add direct LAN and established peer networking.
...
See README.md for runnable components and limitations. Update this section and architecture decisions when scope changes. The backend and Go agent now support enrollment, persistent identity, CPU/RAM inventory, heartbeat sequencing, reconnection, and revocation. Real-agent integration tests exercise PostgreSQL and controller restarts. Native storage now supports immutable folder copies, SQLite transfer recovery, checksums, listing and retrieval through a direct TLS-capable agent endpoint and CLI. Storage supports controller-issued, expiring node/collection-scoped grants and checks revocation on each request. Shared keys are restricted to loopback development. The controller now keeps a named catalogue with pending/agent-confirmed states. Managed CLI copy/catalog/get commands handle scoped grants and progress, and scan once per copy invocation. Small-file transfers now negotiate bounded batches with durable group commits. Managed copies retry transient failures with saved-offset reconciliation, scanning/speed feedback is visible, and `run --root` combines storage and heartbeat lifecycle. Paired remote storage now uses an outbound HTTPS relay with end-to-end device-key verification, isolated relay tickets, and resumable retrieval. Local Windows process validation covers pairing recovery, relay restart, corrupted partial downloads, and revocation; see docs/remote-access.md. Compute persists explicitly placed jobs/applications and their revisions and observations; the agent reconciles them through a restricted Docker executor with digest-pinned images, resource limits, stable volumes and read-only collection inputs. Successful job output is exported, checksum verified, published as an immutable native collection, and exposed through the existing catalogue/retrieval flow. CI now builds a Docker-ready, digest-verified WSL2 rootfs, and the Windows CLI imports, checks, retries, and safely rolls back only a newly created environment. Authenticated application routing now uses workload-bound tickets, isolated relay queues, an executor-owned container proxy, paired-device TLS, and a loopback-only operator listener. The agent can install as an auto-start, recovery-configured Windows service under the same DPAPI user. Current-user Windows DPAPI identity reload and heartbeat sequencing were exercised over SSH on the IdeaPad. The remote storage suite did not finish successfully; see docs/testing/windows-storage-experience.md. The optimized transfer path now passes local Windows race tests, compiled-agent/controller integration, and live copy/retrieval/interruption checks; see docs/testing/codebase-audit.md. Public WAN, remote-device compute, WSL/Docker lifecycle, and Windows service reboot/logout/sleep validation remain pending.

Opted-in paired nodes publish fresh private-LAN storage candidates; managed clients authenticate direct routes before selecting them and fall back to the relay. Controller and relay rate limits and metrics are integrated with workload routes. See [ADR 0013](docs/decisions/0013-connectivity-foundation.md).

The guided setup command now composes pairing, storage initialization, optional verified compute provisioning, and optional service hosting into a resumable host flow. CI publishes those components as one Windows artifact. The self-hosted cloud stack packages private PostgreSQL, migration-gated control-plane startup, Caddy HTTPS, and the native TLS relay with file-mounted secrets.
